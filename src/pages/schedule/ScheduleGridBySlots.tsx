   ...
   return (
     <div className="rounded-lg border bg-card overflow-auto">
       <div
         className="grid"
         style={{
           gridTemplateColumns: `minmax(80px, 100px) minmax(60px, 80px) repeat(${classes.length}, minmax(180px, 1fr))`,
         }}
       >
         ...
         {slots.map((slot) =>
           days.map((day, dayIdx) => {
             return (
               <div key={`${slot}-${day}`} className="contents">
                 {dayIdx === 0 ? (
                   <div
                     key={`slot-${slot}`}
                 ...
                 <div key={`day-${slot}-${day}`} className="border-b p-3 text-sm text-muted-foreground">
                   {day}
                 </div>
                 {classes.map((cls) => {
